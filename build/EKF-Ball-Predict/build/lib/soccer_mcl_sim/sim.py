...
class SoccerSim(Node):
    ...
    def step(self):
        ...
        b_vx = self.ekf_ball_vel[0] if self.ekf_ball_vel else self.ball.vx
        b_vy = self.ekf_ball_vel[1] if self.ekf_ball_vel else self.ball.vy
        ...
        v_total = math.hypot(b_vx, b_vy)

        if b_vx < -10.0:
            friction_decel = 80.0 
            time_to_stop = v_total / friction_decel if friction_decel > 0 else 999.0

            t_hit_ideal = (bx - KEEPER_X) / abs(b_vx)

            if t_hit_ideal > time_to_stop:
                self.robot.vy = 0.0 
            else:
                y_impact = by + (b_vy * t_hit_ideal)
